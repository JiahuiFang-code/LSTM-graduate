...
model.add(Dense(yt,activation='linear',kernel_regularizer=regularizers.l2(0.01)))#,kernel_regularizer=regularizers.l1(0.001)正则化层的添加，权重参数w添加L1正则化；
...
# 神经网络模型训练
history = model.fit(x_train, y_train, epochs=300)#训练2000次
#模型保存
model.save('model.h5')
...
plt.plot(xa,Y_train[:,0],'b-',label='期望值')
plt.plot(xa,Y_train_pre[:,0],'r-',label='预测值')
plt.title('训练集')
plt.xlabel('样本')
plt.ylabel('价格')
