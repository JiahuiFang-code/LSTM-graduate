...
model.add(LSTM(units = 96,  activation='relu',
               return_sequences = True, input_shape = (xt, 5)))
# model.add(Dropout(0.3))
# model.add(LSTM(units = 60 , activation='relu',
#               return_sequences = True ))
model.add(LSTM(units = 72 , activation='linear',
              return_sequences = False ))
# model.add(Dropout(0.3))#dropout 正则化
model.add(Dense(24,activation='relu'))
model.add(Dense(yt,activation='linear',activity_regularizer=regularizers.l1_l2(0.001)))#,kernel_regularizer=regularizers.l1(0.001)正则化层的添加，权重参数w添加L1正则化；
...
# 神经网络模型训练
history = model.fit(x_train, y_train, epochs=400)#训练2000次
#模型保存
model.save('model.h5')
